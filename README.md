...
| 용어 | 정식 명칭 | 설명 |
|------|---------|------|
| **BD_MGT_SN** | 건물관리번호 (25자리) | 행정안전부가 건물마다 부여한 고유 식별번호. Vworld API의 `bd_mgt_sn` 필드. 예: `1141011300100120000000001` |
| **PNU** | 필지고유번호 (19자리) | 토지(필지) 단위의 고유번호. **Vworld에서는 `bd_mgt_sn[:19]`로 추출.** 시도+시군구+읍면동+리+토지구분+본번+부번. 예: `1111010100100010000` |
...
| 인증 (Referer 헤더) | **PASS** |
| GeoJSON 형태로 폴리곤 좌표 반환 | **PASS** (MultiPolygon) |
| bd_mgt_sn (25자리 건물관리번호) 포함 | **PASS** (`1111012000100050004000007`) |
| PNU = bd_mgt_sn[:19]로 추출 | **PASS** (`1111012000100050004`) |
| 반경 기반 검색 (geomFilter + buffer) | **PASS** (500m 반경, 10건 반환) |
| 일일 호출 제한 확인 | 무제한 (마이페이지 확인) |

- **엔드포인트**: `http://api.vworld.kr/req/data`
- **레이어**: `LT_C_SPBD` (도로명주소 건물) — ~~LT_C_AISRESC~~는 항공장애물제한표면이었음
- **인증**: 서비스타입 "웹사이트"의 경우 `Referer: http://localhost` 헤더 필수
- **속성 필드**: `bd_mgt_sn`, `buld_nm`, `gro_flo_co`, `rd_nm`, `buld_no` 등 (소문자)
- **PNU 추출**: 별도 PNU 필드 없음. `bd_mgt_sn`의 앞 19자리가 PNU
...
```
Vworld LT_C_SPBD (bd_mgt_sn + 폴리곤) → PNU = bd_mgt_sn[:19]
       │
       ├── PNU (19자리) 분해
       │   ├── 시도코드 (2자리)
...
- Vworld 인증: 서비스타입 "웹사이트"에서는 `Referer: http://localhost` 헤더 필수
