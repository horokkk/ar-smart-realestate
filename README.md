...
| 인증 (Referer 헤더) | **PASS** |
| GeoJSON 형태로 폴리곤 좌표 반환 | **테스트 대기** (레이어 수정 완료) |
| bd_mgt_sn (25자리 건물관리번호) 포함 | **테스트 대기** |
| PNU = bd_mgt_sn[:19]로 추출 | **테스트 대기** |
| 반경 기반 검색 (geomFilter + buffer) | **테스트 대기** |
...
| API | 키 발급 | 테스트 | 비고 |
|-----|:------:|:------:|------|
| Vworld 건물통합정보 (LT_C_SPBD) | O | **부분 PASS** | 인증 성공, 레이어 수정 완료. 재실행 필요 |
...
### 아직 안 되는 것
- Vworld — 인증 성공, `LT_C_SPBD` 레이어로 수정 완료. 재실행하여 bd_mgt_sn/폴리곤 확인 필요
